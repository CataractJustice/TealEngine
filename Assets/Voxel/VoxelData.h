...
public:
	...
	EventPublisher onModify;
	ScalarFieldWrapping wrapMode;
	Voxel defaultValue;

	VoxelData(ivec3 dimensions, ivec3 index = ivec3(0), VoxelWorld* world = nullptr, ScalarFieldWrapping wrapMode = NO_WRAP);
	~VoxelData();
	ivec3 getDimensions();
	ivec3 getIndex();
