...
namespace TealEngine 
{
	BackgroundColor::BackgroundColor() 
	{
		...
		addProp(new ColorProp(glm::value_ptr(pressColor)), "On press color");
	}

	void BackgroundColor::GUIRender() 
	{
		...
		float offsety 	= 1.0f - transform.getTop().inWindowHeights(transform);
		...
		shader.setUniform("color", 	this->getParent()->getMouseButtonPressMask() ? pressColor : color);
	...
	EXPORT_COMPONENT(BackgroundColor);
}